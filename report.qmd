...
This report presents the first known systematic study of deploying PI0.5 via the **VLASH** framework [@han2025vlash] on a AGILE X Robotics Piper 6-DOF arm running on an NVIDIA Jetson AGX Orin, targeting a **ball pick-and-place** task. To the best of our knowledge, this is the first deployment of the PI0.5 model on Jetson AGX Orin embedded hardware. The contributions documented here are:

- **Integration:** five targeted compatibility shims bridging VLASH against the `lerobot_piper` robot driver on `linux-aarch64`.
- **Training configurations:** three fine-tuning regimes (async + LoRA, sync, sync + LoRA) with fully specified hyperparameters.
- **Latency benchmarks:** empirical measurements showing a 29.5× reduction in mean per-inference time (184.5 ms vs. 5444.1 ms) and a 2.9× reduction in task completion time (52 s vs. 153 s) for asynchronous inference relative to synchronous inference on a reference desktop GPU.
...
**Our contribution**: Systematic intergration of a turnkey solution to deploy VLAs on realistic edge hardware
...
| **Task** | Multi-task generalization | Sequencial task black and white ball sorting (proof-of-concept) |
...
Demonstrations were collected via **kinesthetic teaching**: an operator physically guides the arm through the pick-and-place motion while the system records joint states and camera streams. Each episode was triggered manually at the start o a clean arm home pose and terminated upon successful placement.
...
All models reported here were trained on dataset, consisting of demonstrations of black and white ball pick-and-place task collected via kinesthetic teaching. Observations include wrist (`wrist`) and overhead (`up`) RealSense streams at 640×480, and the six-dimensional joint state vector. All models use `state_cond: true`.
...
On the inference side, latency benchmarks confirm that asynchronous inference with TDA reduces per-call latency by 29.5× and task completion time by 2.9× relative to synchronous inference on a reference desktop GPU. Crucially, asynchronous inference also runs correctly on the Jetson AGX Orin, meaning the full VLASH pipeline — TDA training, LoRA fine-tuning, and async deployment — is validated end-to-end on embedded hardware.
